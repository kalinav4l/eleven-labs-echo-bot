            ...
            <div className={`text-center transition-all duration-500 ${(isRecording || isConnected) ? 'h-96 flex flex-col justify-between' : 'space-y-8'}`}>
              <div className={`transition-all duration-500 ${(isRecording || isConnected) ? 'flex-1 flex items-center justify-center relative' : ''}`}>
                ...
                {(isRecording || isConnected) && (
                  <>
                    ...
                    <div
                      ...
                      style={{
                        width: `${140 + audioLevel * 150}px`,
                        height: `${140 + audioLevel * 150}px`,
                        opacity: Math.min(audioLevel * 1.2, 1)
                      }}
                    />
                  </>
                )}
                ...
                <div 
                  ...
                  style={{
                    transform: (isRecording || isConnected) ? `scale(${1.1 + audioLevel * 0.6})` : undefined,
                    boxShadow: (isRecording || isConnected) ? `0 0 ${30 + audioLevel * 80}px rgba(${isConnected ? '34, 197, 94' : '59, 130, 246'}, ${0.6 + audioLevel * 0.8})` : undefined,
                    filter: (isRecording || isConnected) ? `brightness(${1 + audioLevel * 0.5})` : undefined
                  }}
                >
                ...
                <>
                  ...
                  >
                    <div className="flex items-center justify-center space-x-2">
                      ...
                      <span>{isConnecting ? 'Se conectează...' : `Începe Demo cu ${voiceOptions[selectedVoice].name}`}</span>
                    </div>
                  </button>
                </>
              )}
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-center space-x-3 mb-4">
                    ...
                    <span className="text-brand-100 text-sm font-medium">
                      {isConnected ? `Conectat la Kalina AI (${voiceOptions[selectedVoice].name})` : 'Conectare...'}
                    </span>
                  </div>
                  ...
                  <button 
                    className={`bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white border-2 border-red-400 shadow-red-500/30 transform hover:scale-105 btn-magnetic text-lg px-8 py-4 mx-auto transition-all duration-300 rounded-xl font-semibold shadow-lg ${
                      audioLevel > 0.01 ? 'animate-pulse' : ''
                    }`}
                    onClick={handleDemoClick}
                    style={{
                      boxShadow: `0 0 ${20 + audioLevel * 50}px rgba(239, 68, 68, ${0.5 + audioLevel * 1.0})`,
