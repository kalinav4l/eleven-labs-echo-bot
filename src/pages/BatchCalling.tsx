...
import { Plus, Phone, Users, MoreHorizontal, Play, Pause, RotateCcw, Eye } from 'lucide-react';
      ...
      <div className="min-h-screen bg-gray-950 text-white">
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-3xl font-bold text-white">Apeluri Batch</h1>
              <p className="text-gray-400 mt-2">Gestionează și monitorizează campaniile de apeluri</p>
            </div>
            <Button 
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <Card className="bg-gray-900 border-gray-800">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-gray-400">Total Loturi</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold text-white">{batchCalls?.length || 0}</div>
              </CardContent>
            </Card>
            
            <Card className="bg-gray-900 border-gray-800">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-gray-400">În Progres</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card className="bg-gray-900 border-gray-800">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-gray-400">Finalizate</CardTitle>
              </CardHeader>
              <CardContent>
          ...
          <Card className="bg-gray-900 border-gray-800">
            <CardHeader>
              <CardTitle className="text-xl text-white">Istoricul Loturilor</CardTitle>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-gray-800">
                        <th className="text-left py-3 px-4 text-sm font-medium text-gray-400">Nume Campanie</th>
                        <th className="text-left py-3 px-4 text-sm font-medium text-gray-400">ID Lot</th>
                        <th className="text-left py-3 px-4 text-sm font-medium text-gray-400">Creat</th>
                        <th className="text-left py-3 px-4 text-sm font-medium text-gray-400">Status</th>
                        <th className="text-left py-3 px-4 text-sm font-medium text-gray-400">Destinatari</th>
                        <th className="text-right py-3 px-4 text-sm font-medium text-gray-400">Acțiuni</th>
                      </tr>
                    </thead>
                    <tbody>
                      {batchCalls?.map((batch) => (
                        <tr key={batch.id} className="border-b border-gray-800 hover:bg-gray-800/50">
                          <td className="py-4 px-4">
                            <div className="font-medium text-white">{batch.call_name || 'Lot de apeluri'}</div>
                          </td>
                          <td className="py-4 px-4">
