...
const executeAddContact = async (userId: string, name: string, phone: string, company?: string, email?: string, notes?: string) => {
  try {
    const { data, error } = await supabase
      .from('contacts_database')
      .insert({
    ...
    if (error) {
      return {
        success: false,
        message: `Eroare la adăugarea contactului: ${error.message}`,
        data: null
      };
    }
    ...
    return {
      success: true,
      message: `Contactul ${name} (${phone}) a fost adăugat cu succes în baza de date`,
      data: data
    };
  } catch (error) {
    return {
...
const executeCreateCampaign = async (userId: string, campaignName: string, description: string, targetContacts?: string[]) => {
  try {
    const { data, error } = await supabase
      .from('campaigns')
      .insert({
    ...
    if (error) {
      return {
        success: false,
        message: `Eroare la crearea campaniei: ${error.message}`,
        data: null
      };
    }
...
const executeExportData = async (userId: string, dataType: string, dateRange?: string) => {
  try {
    let data = [];
    let fileName = '';
    ...
    switch (dataType) {
      case 'calls':
        const calls = await getAllCallHistory(userId);
        data = calls;
        fileName = 'call_history_export.json';
        break;
      case 'contacts':
        const contacts = await getUserContacts(userId);
        data = contacts;
      ...
      default:
        return {
          success: false,
          message: `Tip de date necunoscut: ${dataType}. Tipuri disponibile: calls, contacts, conversations, transactions`,
          data: null
        };
    }
    ...
    return {
      success: true,
      message: `Am exportat ${data.length} înregistrări de tip "${dataType}". Datele sunt disponibile pentru descărcare.`,
      data: {
        fileName,
        recordCount: data.length,
        exportData: data.slice(0, 100) // Limit preview to first 100 records
      }
    };
  } catch (error) {
    return {
      success: false,
      message: `Eroare la exportul datelor: ${error.message}`,
      data: null
    };
  }
};

const executeGetAnalytics = async (userId: string, period: string = '30d') => {
  try {
    ...
    const periodCalls = calls.filter(call => new Date(call.created_at) >= periodStart);
    const successfulCalls = periodCalls.filter(call => call.call_status === 'completed');
    const failedCalls = periodCalls.filter(call => call.call_status === 'failed');
    
    const totalCost = periodCalls.reduce((sum, call) => sum + (Number(call.cost_usd) || 0), 0);
    const totalDuration = periodCalls.reduce((sum, call) => sum + (Number(call.duration_seconds) || 0), 0);
    const avgDuration = periodCalls.length > 0 ? totalDuration / periodCalls.length : 0;
    const successRate = periodCalls.length > 0 ? (successfulCalls.length / periodCalls.length) * 100 : 0;
    
    return {
      success: true,
      message: `Analiză completă pentru perioada ${period}`,
      data: {
        period,
        ...
        totalDuration: Math.round(totalDuration / 60) + ' minute',
        avgDuration: Math.round(avgDuration / 60) + ' minute',
        analytics: analytics.slice(0, 5) // Top 5 conversation analytics
      }
    };
  } catch (error) {
    return {
      success: false,
      message: `Eroare la generarea analizei: ${error.message}`,
      data: null
    };
  }
};
...
const tools = [
  ...
  },
  {
    ...
    function: {
      name: "add_contact",
      description: "Adaugă un contact nou în baza de date",
      parameters: {
        type: "object",
        properties: {
          name: {
          ...
          notes: {
            type: "string",
            description: "Note despre contact (opțional)"
          }
        },
        required: ["name", "phone"]
      }
  ...
  },
  {
    ...
    function: {
      ...
      description: "Creează o campanie nouă de marketing/vânzări",
      parameters: {
        type: "object",
        properties: {
          campaign_name: {
          ...
          target_contacts: {
            type: "array",
            items: { type: "string" },
            description: "Lista de contacte țintă (opțional)"
          }
        },
        required: ["campaign_name", "description"]
      }
    }
  },
  {
    ...
    function: {
      name: "export_data",
      description: "Exportă date din sistem (apeluri, contacte, conversații, tranzacții)",
      parameters: {
        type: "object",
        properties: {
          data_type: {
            type: "string",
            enum: ["calls", "contacts", "conversations", "transactions"],
            description: "Tipul de date de exportat"
          },
          date_range: {
            type: "string",
            description: "Perioada de export (opțional)"
          }
        },
        required: ["data_type"]
      }
  ...
  },
  {
    ...
    function: {
      name: "get_analytics",
      description: "Generează analize detaliate și rapoarte de performanță",
      parameters: {
        type: "object",
        properties: {
          period: {
            type: "string",
            enum: ["1d", "7d", "30d", "90d"],
            description: "Perioada pentru analiză",
            default: "30d"
          }
        }
      }
      ...
      for (const toolCall of aiMessage.tool_calls) {
        ...
        try {
          ...
          switch (toolCall.function.name) {
            ...
            case 'add_contact':
              toolResult = await executeAddContact(
                userId,
                args.name,
                args.phone,
                args.company,
            ...
            case 'create_campaign':
              toolResult = await executeCreateCampaign(
                userId,
                args.campaign_name,
                args.description,
            ...
            case 'export_data':
              toolResult = await executeExportData(
                userId,
                args.data_type,
                args.date_range
              );
            ...
            case 'get_analytics':
              toolResult = await executeGetAnalytics(
                userId,
                args.period
              );
              break;
