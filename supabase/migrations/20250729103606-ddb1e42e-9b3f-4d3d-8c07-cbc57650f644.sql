-- Script mai sigur pentru a deduce retroactiv costurile pentru conversațiile care nu au fost procesate
...
BEGIN
    ...
    FOR conversation IN 
        SELECT user_id, conversation_id, duration_seconds, cost_credits, call_date
    ...
    LOOP
        ...
        IF calculated_cost > 0 THEN
            ...
            PERFORM deduct_balance(
                ...
                NULL  -- Nu trimitem conversation_id ca UUID
            );
            ...
            RAISE NOTICE 'Procesat %: User % - Conversație % - Cost: $% - Durată: %s', 
                processed_count, conversation.user_id, conversation.conversation_id, calculated_cost, conversation.duration_seconds;
        END IF;
    END LOOP;
    
    RAISE NOTICE 'Finalizat! Procesate % conversații retroactiv.', processed_count;
END $$;